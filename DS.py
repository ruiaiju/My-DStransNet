#如何利用自己训练好的deeplab v3+模型进行预测，一般只修改model—path和num—classes
import colorsys
import copy
import time
import cv2
...
class DeeplabV3(object):
    _defaults = {
        ...
        #   训练好后logs文件夹下存在多个权值文件，选择验证集损失较低的即可。将这个文件复制过来即可
        #   验证集损失较低不代表miou较高，仅代表该权值在验证集上泛化性能较好。
        #-------------------------------------------------------------------#
        "model_path"        : 'model_data/deeplab_mobilenetv2.pth',#比如'logs/权值文件的文件名.pth'
        #----------------------------------------#
        #   所需要区分的类的个数+1
        #----------------------------------------#
        "num_classes"       : 21,#与train.py里面修改的num_classes保持一致
        #----------------------------------------#
        #   所使用的的主干网络：
        #   mobilenet
        #   xception    
